...
class SkillDataset(Dataset):
    ...
    def __len__(self):
        ...
        # if self.debug:
        #     logging.info(f"Dataset length (excluding last problems): {length}")
        return length
    ...
    def __getitem__(self, idx):
            ...
            for i, submission_idx in enumerate(submission_indices):
                ...
                # if self.debug:
                #     print(f"Current submission time: {current_submission_time}")
                #     print(f"Previous submission time: {prev_submission_time}")
    ...
    def _embed_questions_with_codet5(self, help_center_questions):
        """
        Arg: help_center_questions (해당 제출물을 제출하기 전에 했던 질문 내역들)
    ...
    def _fetch_random_question(self):
        ...
        return: 랜덤으로 선택한 학생의 질문과 그에 대한 교사의 응답 (string)
        """
        #학생의 질문만 남도록 필터링
    ...
    def _embed_skills(self, questions):
        ...
        Returns:
            torch.Tensor: 추출된 스킬의 임베딩이 모두 포함된 텐서. 위아래로 연결됨.
    ...
    def _process_skill(self, content):
        ...
        Returns:
            torch.Tensor: 추출된 스킬을 임베딩한 텐서. 스킬이 없을 경우 기본 임베딩을 반환.
    ...
    def _embed_skill(self, skill_text):
        ...
        Returns:
            torch.Tensor ([1, 512]): 스킬 텍스트에 대한 임베딩 벡터.
    ...
    def _embed_next_skills(self, ex_id):
        ...
        Returns:
            torch.Tensor ([1, 512]): 추출된 스킬 텍스트에 대한 임베딩 벡터
        """
        max_length = 512
