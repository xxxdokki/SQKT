...
# JSON 로그 파일 설정
JSON_LOG_FILE = "student_problem_logs.json"

def log_batch_info_as_json(loader, phase):
    with open(JSON_LOG_FILE, "a") as log_file: 
        for batch_idx, data in enumerate(loader):
            _, _, _, _, _, _, _, _, _, _, original_data = data
            for record in original_data:
                student_id = record['user_id']
                problem_id = record['problem_id']
                problem_order = record['problem_order']
                submissions = len(record['code_submissions'])  
...
def main():
    ...
    log_batch_info_as_json(train_loader, phase="Training")
    log_batch_info_as_json(val_loader, phase="Validation")
    log_batch_info_as_json(test_loader, phase="Evaluation")
